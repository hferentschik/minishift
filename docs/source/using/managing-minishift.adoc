...
Running `minishift ssh` without a sub-command opens an interactive shell.
You can then run commands on the Minishift VM in the same way that you run commands interactively on any remote machine using SSH.

You can also run `minishift ssh` with a sub-command. In this case the command is sent to the Minishift VM and the result returned to your local shell. For example:
