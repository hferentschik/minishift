...
Minishift provides a persistent-volumes <<addons#,add-on>> which allows you to create an initial set of persistent volumes.
After installing and enabling the add-on, it will provision 5 persistent volumes of type _hostPath_ during <<minishift-start,minishift start>>.
Enabling this add-on will allow applications to make link:https://docs.openshift.org/latest/dev_guide/persistent_volumes.html#persistent-volumes-claims-as-volumes-in-pods[persistent volumes claims].
